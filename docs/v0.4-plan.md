...
- Completion boundary: The next clean successor after rejected candidate
  `02e23c59bddacd641d4bb645d39b9c9298f4990a` must retain the
...
Current dirty successor bytes add a bounded fail-closed signature check for
tokenizer quote failure and CMD caret normalization while preserving a
well-formed foreign command that merely prints `aoi-codex-hook`. Targeted
onboarding/offboard regression evidence is Windows `68 passed, 4 skipped, 49
subtests` and WSL `69 passed, 3 skipped`. Linux/Win32 typing over 87 production
files, compileall, strict MkDocs, packaged-policy byte equality, and diff-check
also pass. These are dirty-byte pre-commit checks, not clean-commit, package,
canary, or promotion evidence. The detector remains a bounded
direct-token/known-shell guardrail, not a general shell-equivalence engine or
DLP. A fresh read-only reviewer packet accepted these dirty bytes with
P0/P1/P2=`0/0/0` for a local clean successor commit only. Its routing remains
`manual_unverified` because the collaboration platform exposes no consumable
`SubagentStart` receipt. No older seal, doctor PASS, package, test result, or
canary authorization qualifies the eventual successor. No successful live
Codex turn, final integrity seal, encrypted local bundle, downstream
installation, or ARISE workload is claimed here. This task forbids remote CI
and publication rather than treating them as missing acceptance evidence.
