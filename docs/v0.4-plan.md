...
Status: active implementation contract, evidence checkpoint 2026-07-21
...
- Completion boundary: The containing clean successor after the v49 rejection
  must retain every accepted `be46e89` contract while adding three exact
  repairs: process- and thread-level `local_files` controls bound to an isolated
  `CODEX_HOME`; a live `model/list` gate for the exact visible requested
  model/effort before `thread/start`; and synchronous task-local CAS retention
  of every correlated rejected response before its fault is journaled. The
  successor must also retain the canonical WSL direct/Windows-`wsl.exe`
  provenance-bound hook pair and the exact pre-turn Git/tree/status/full-claim
  recapture for both sandbox modes. That exact successor must pass bridge/
  confidentiality falsification, pinned runtime/schema checks, full Windows
  and fresh WSL suites, typing/compile/docs, exact-wheel package/install,
  a newly issued read-only then writable live canary, verified Git mutation,
  profile-aware doctor, fresh independent review, integrity-v2 seal, and an
  encrypted local bundle. No push, remote CI, release, package/artifact
  publication, connector export, or ARISE RTL/EDA execution. Only after those
  gates may ARISE AOI be backed up and the exact reviewed version installed
  side-by-side with Windows-to-WSL hook reachability, then stop.
...
This v49 repair candidate is deliberately pre-canary. Production
...
thread-started, or turn-started milestone. Current
At the pre-commit checkpoint, targeted evidence was diagnostic on dirty bytes:
...
not promotion evidence, and do not authorize v13: source still needs a clean
successor, complete fresh gates, and independent review.
...
1. **Contract tests:** historical exact `be46e89` evidence passed its complete
   ...
   install gates. The current dirty v49 repair has targeted fake-runtime,
   contract, CAS, and process-policy evidence only. No result transfers to its
   eventual clean successor; that commit must rerun every local gate.
