...
1. **Contract tests** are the focused/static results above plus the still
...
2. **Live App Server canary** is still absent on this successor. The exact
   ...
   The canary v2 spec additionally binds the exact isolated `CODEX_HOME`
   policy and Bridge executable bytes; each Bridge subprocess revalidates those
   bytes, overrides ambient `CODEX_HOME`, and scrubs AOI authority plus known
   publication credentials and Python path/home/venv influence. It disables
   the user site and unsafe path insertion while retaining model-service
   authentication. The trusted SessionStart receipt and final canary evidence
   must additionally bind the installed wheel/RECORD/runtime provenance; the
   launcher hash alone is not a claim about imported module bytes. The
   rejected external v1 preparation driver is diagnostic history only and
   must not be resumed.
...
remote or URL rewrite, and rejects an `ARISE` root. The read-only phase requires
an identical pre/post workload snapshot; the writable phase requires a real
workload delta plus separate `verified_mutation` elevation. Structural tests
use a fake bridge only. No live App Server canary has been run on this
successor, and none is authorized until the exact stable runtime/schema pin is
requalified.
