...
The repair is a wire-breaking disposable-canary contract change and therefore
uses `aoi.codex-transport-canary.v3`, not a silent reinterpretation of v2.
The spec and result bind the exact Git executable path, byte count, and SHA-256
in addition to the Codex runtime and Bridge. Codex, Bridge, and Git executable
checks share a bounded 1 MiB-chunk streaming hash with a 1 GiB ceiling and
before/open/after file-identity checks; Codex and Bridge are revalidated before
every Bridge subprocess and Git is revalidated before every Git subprocess.
...
2. **Live App Server canary** is still absent on this successor. The exact
   ...
   The canary v3 spec additionally binds the exact isolated `CODEX_HOME`,
   ...
   wheel/RECORD/runtime provenance; executable hashes alone are not a claim
   about imported module bytes. Rejected v1/v2 preparation assets are
...
The disposable canary driver is opt-in: its default performs only a reserved
launch preflight, and `--execute` is required to consume a permit. It requires
an exact runtime pin, a committed marker-bearing scratch repository with no
remote or URL rewrite, a bounded self-contained Git metadata inventory, and
rejects an `ARISE` root. Repository config, common-dir/object/ref routing,
alternates, grafts, replacement refs, shallow state, system attributes,
nested links/reparses, and executable provenance all fail closed before
evidence is accepted. The read-only phase requires an identical pre/post
workload snapshot; the writable phase requires a real workload delta plus
separate `verified_mutation` elevation whose v2 semantic object references the
exact Bridge-side Git binding. Structural tests use a fake bridge except for
the real local Git plumbing and CLI mutation tests. No live App Server canary
has been run on this successor, and none is authorized until the exact stable
runtime/schema pin is requalified.
