...
The successor exact-test producer uses runner version 2 and the explicit
`pytest-contained-argv-v2` contract. New evidence records both requested and
canonical effective argv and binds a runner-generated empty pytest config,
snapshot root/confcutdir/cwd roles, environment digest, plugin-autoload
disablement, and `PYTHONNOUSERSITE=1`. The caller grammar is limited to `-q`,
one enumerated `--tb=...`, contained `--ignore=...` paths, and exact-spelling
...
collection fail before pytest starts. Historical version-1 receipts remain
parseable by integrity/doctor replay, but every new `add-verification` exact
evidence ingestion requires the current version-2 protocol.
