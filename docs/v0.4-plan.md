...
- Completion boundary: The containing clean successor after the exact
  `ff8fd223` fresh-ext4 WSL failure must retain every accepted production
  contract from that commit while preserving its three v49
  ...
  successor may add only a tests-only Codex issue clock driver and causally
  anchored reachability fixture time; production liveness, permit, expiry, and
  tolerance bytes remain unchanged. It must also retain the canonical WSL direct/Windows-`wsl.exe`
