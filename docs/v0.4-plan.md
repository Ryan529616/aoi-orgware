...
**2026-07-23 remote publication update:** `v0.4.0a1` was pushed to GitHub for
remote-final-SHA validation and immediately superseded before Release/PyPI
because the remote matrix exposed two CI portability defects: a Python 3.11
test-collection failure from a backslash inside an f-string expression, and a
Windows package-smoke failure where `PYTHONPATH=src` was evaluated after
entering the downloaded artifact directory. No GitHub Release, PyPI upload, or
ARISE install may use `v0.4.0a1`. The publication candidate is retargeted to
`0.4.0a2` on a successor commit with the Python 3.11-compatible fixture and
absolute Windows workflow `PYTHONPATH` repair.
