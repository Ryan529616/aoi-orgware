...
name, version, and allowed artifact hashes of all 32 locked distributions
(40 wheel hashes total). Build the
...
The release lock now contains 32 distributions and 40 allowed wheel hashes,
including `twine==6.2.0` and all selected transitive dependencies for the
Windows/Linux Python 3.11--3.14 release runners. `test.yml` and every
...
Current dirty-successor evidence is intentionally limited to focused Windows
contract tests: exact-test/CLI-CAS is 85 passed, 1 skipped, 19 subtests; release
rehearsal/workflow contracts are 44 passed, 1 skipped. Targeted mypy,
compileall, and `git diff --check` pass. This is not exact-commit evidence.
Independent repair review, a clean successor commit, rebuilt package/install
evidence, exact Windows/coverage/fresh-ext4 WSL suites, live App Server
canaries, integrity-v2 seal, doctor, exact-main remote test/docs, publication
readback, promotion, and ARISE installation remain mandatory.
