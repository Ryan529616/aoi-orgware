...
Status: active implementation contract, evidence checkpoint 2026-07-24
...
- Completion boundary: A clean containing successor of rejected exact commit
  `054d1b6cf877fda8d24372318508614be28efe98` must retain all accepted Bridge,
  `local_files`, WSL-hook, CAS, exact-model-list, clock-fixture, and
  integrity-v2 contracts. Every `model/rerouted` notification must
  synchronously bind its exact bounded wire bytes to controller-owned
  task-local CAS before payload classification and then fail closed with a
  typed fault. A `turn/completed` observation-derived terminal append requires
  a natural zero-exit/full-reader-drain stream seal; forced cleanup, nonzero
  exit, partial output, reader/CAS timeout, or a live reader may never authorize
  `completed`. That exact successor must pass Bridge/confidentiality
  falsification, pinned runtime/schema checks, full Windows and local
  fresh-ext4 WSL suites, typing/compile/docs, exact-wheel package/install, a
  newly issued read-only then writable live canary, verified Git mutation,
  profile-aware doctor, fresh independent review, integrity-v2 seal, and an
  encrypted local bundle. `local_files` is selective: an empty protected-rule
  set permits normal AOI publication, `home_remote_only` permits only the exact
  named home remote and destination, and `local_only` forbids all external
  destinations absent an exact consumed Chief one-shot export permit. This
  same active task must push the exact sealed final commit after an exact
  pre-push check. Canonical GitHub `test` Linux/Windows jobs and the main-only
  `docs` workflow must then pass for the exact peeled `main` push commit. Their
  authenticated exact-CI receipt must be copied into task CAS before an unused
  annotated release tag is created. A second CAS receipt must bind that exact
  CI record, current plan/head, tag object/peeled commit, destination, remote
  absence, and confidentiality preflight before tag push; authenticated
  post-push tag-object/peeled-commit readback is a third distinct receipt. A
  tag test is supplementary and cannot replace the main-push observations.
  The publish workflow must independently fail closed on those exact runs
  before its first GitHub Release mutation. It must publish and read back the
  immutable tag,
  GitHub Release assets, and exact wheel/sdist through GitHub OIDC Trusted
  Publishing, then verify PyPI metadata, hashes, download, install, import, and
  entry points. Only after the current Chief binds those public observations
  through `release-promote` may ARISE consume the resulting exact promotion
  bundle and canonical bundle SHA, install the released version side-by-side,
  and prove Windows-to-WSL hook reachability. ARISE source, Git, RTL, artifacts,
  and EDA remain outside publication and execution scope.
...
## 2026-07-25 Linux qualification-gate repair

Exact commit `71f2944052ccdf4914cbf7a9cbe5840ce1b0df74` was rejected by the
canonical Linux coverage gate with
`3 failed, 2333 passed, 46 skipped, 490 subtests passed in 6567.78s`. Two
independent defects were found, both in qualification tooling rather than in
product behaviour, and both invisible to the Windows-only evidence recorded for
that candidate.

**Defect 1 — the coverage lock could not resolve a compiled tracer on Linux.**
`requirements/coverage-tools.lock` declares `--only-binary=:all:` and pinned
`coverage==7.15.2` to only `cp314-cp314-win_amd64` and the pure-Python
`py3-none-any` wheel. No Linux interpreter can satisfy that from a compiled
wheel, so both the fresh-ext4 WSL gate and the `ubuntu-latest` coverage job
resolved the pure-Python wheel. The gate's own `01-setup.log` records
`coverage-7.15.2-py3-none-any.whl` being installed and its stderr log records
`CoverageWarning: Couldn't import C tracer: No module named 'coverage.tracer'
(no-ctracer)`. That warning breaks
`test_release_cli.py::ReleaseCliTests::test_manifest_observation_stdout_is_exact_canonical_without_newline`,
which asserts an empty child stderr; the pure tracer's overhead breaks
...
The lock now pins the compiled `cp313`/`cp314` wheels for `manylinux_x86_64`
and `win_amd64` and deliberately omits `py3-none-any`: that wheel resolves
everywhere, so pinning it converts a missing platform into a silent
pure-tracer degradation rather than an install failure. The coverage job
asserts `import coverage.tracer` immediately after install so the same class of
regression fails in seconds. Verified on Linux/CPython 3.13.5: the lock
resolves
`coverage-7.15.2-cp313-cp313-manylinux1_x86_64.manylinux_2_28_x86_64.manylinux_2_5_x86_64.whl`,
`coverage.tracer` imports, and a measured child process emits empty stderr.

**Defect 2 — the authoritative typing gate was never satisfied.**
`src/aoi_orgware/windows_handle_identity.py`, introduced by `d8da4ed`, was
checked only against a Windows-resolved mypy. The Ubuntu-only hash-locked gate
is authoritative, and on exact `29e088b` it reported 22 `[attr-defined]`
errors: Linux typeshed has no `ctypes.WinDLL`, `ctypes.WinError`,
`ctypes.get_last_error`, or `msvcrt.get_osfhandle`. The 16 `os.name`-guarded
call sites now carry `# type: ignore[attr-defined]`, the reviewed pattern
`4b0de37` established for the `msvcrt`/`ctypes.windll` branches; no mypy
configuration enables `warn_unused_ignores`, so the annotation is inert on the
emulated `win32` pass. Verified with the exact
`requirements/typecheck-tools.lock` toolchain (mypy 2.3.0, compiled) on
Linux/CPython 3.13: both the native and `--platform win32` passes report
`Success: no issues found in 95 source files`.

Neither repair changes runtime behaviour. `scripts/combine_coverage.py` records
the new lock digest in its provenance receipt, and only the 16 annotated lines
differ from the parent blob of `windows_handle_identity.py`.

This section records the two repairs and their exact local evidence only. Full
Windows and fresh-ext4 WSL suites, the coverage floor, the canonical
`test.yml`/`docs.yml` main-push runs at one exact SHA, package/install
acceptance, independent review, integrity-v2 seal, tag creation, GitHub
Release, PyPI publication, and ARISE installation all remain required and are
not claimed here.

## 2026-07-24 release-successor integration checkpoint

The containing successor integrates the independently qualified legacy
resource-config compatibility commit, the Codex Transport V2 successor, and the
typed semantic claim/checkpoint lifecycle in that order. The clean integration
baseline was commit
`6e89c756054e8eab0bb013888d0a29c127fa3580`, tree
`ca9b2bad10db1c9908a50178f6e1a47bca505b4a`. Predecessor PASS and review
records remain slice-scoped; none was relabelled as containing-tree evidence.

Baseline-scoped integrated Windows focused evidence includes:

- resource-config compatibility: 49 tests passed; session receipt/registration
  follow-up: 41 tests and 14 subtests passed;
- Transport contracts and stdio/CLI/runtime surfaces: 291 tests, 2 skips, and
  6 subtests passed when the disjoint groups are combined;
- semantic claims, checkpoint/persistence, migration, crash consistency, and
  reachability: 82 tests and 40 subtests passed;
- release/confidentiality/static selection: 173 tests, 3 skips, and 2 subtests
  passed; the corrected pair of workflow-contract files passes 32 tests;
- native and emulated-win32 mypy pass all 95 package source files; compileall
  and diff checks pass;
- one wheel and one sdist built from the clean integration baseline passed
  isolated wheel and sdist-derived-wheel installs plus strict metadata checks.

The release/static shard exposed one stale ordinary-workflow assertion: the
workflow correctly excludes `test_coverage_combine.py` from the dependency-free
unit matrix because that module imports the dedicated hash-locked coverage
toolchain, but the older assertion still required the unsplit command. The
containing successor updates that contract to require the exact exclusion and
to forbid coverage-tool leakage into the unit job. A test found before the
correction is rejected evidence, not a release failure that can be ignored.

A later independent pre-release review found that the live repository config
defaulted to `standard` while the tracked publication snapshot and three
workflow pins still named an older `local_files` digest. That mismatch would
make the real publication preflight fail closed. The successor therefore keeps
the active task's exact unchanged `aoi.toml` and its effective `standard`
authority, regenerates the tracked snapshot from those live bytes, and updates
the `test`, `docs`, and `publish` pins together. This does not change the
opt-in `local_files` contract used by projects with user-designated protected
paths. Every earlier full/package/review result is predecessor evidence after
these byte changes and must be rerun on the next clean commit.

A subsequent source-only pre-release review rejected exact candidate
`34526917b652803a921f07bfacae912f0d9b7576`. Its first pass found P0/P1/P2
`0/2/2`: unbound/ambient-routable Git evidence, Windows junction traversal in
the workload snapshot, leaked GitHub Actions OIDC/runtime publication
authority, and unbounded whole-file executable hashing. A second pass over the
dirty repair found two more P1 gaps and three current P2 gaps: non-config Git
repository authority, ambient Git inside Bridge mutation recapture, mixed
Windows path spelling, an unbounded scratch-marker read, and a stale compile
claim. Its running Windows coverage job was deliberately terminated without a
PASS; its WSL runner was never executed. Those evidence claims were released
rather than transferred to successor bytes.

The current repair is a second wire-breaking disposable-canary contract change
and therefore uses `aoi.codex-transport-canary.v4`, not a silent
reinterpretation of v3. In addition to the v3 Git/runtime executable bindings,
v4 binds the exact local package-gate receipt, source commit/tree, wheel bytes,
PEP 610 archive URL/hash, installed `RECORD`, complete installed
`aoi_orgware`/dist-info namespace, and Bridge launcher row. It rejects import
shadows, `.pth` authority, extra AOI distributions, links/reparses, hard links,
and unrecorded installed files. Codex, Bridge, Git, wheel, and installed-file
checks use bounded streaming hashes with before/open/after file-identity
checks; the complete installed-distribution closure is revalidated before and
after every Bridge subprocess, and Git is revalidated before every Git
subprocess.
All ambient `GIT_*` and SSH agent/askpass variables are removed, then a closed
noninteractive Git configuration is injected. Before and after every Git
command, exact `.git/config` bytes are parsed with `--no-includes`, restricted
to the minimal non-bare/autocrlf/user/platform allowlist, and digest-bound;
include paths, fsmonitor/hooks, filters, diff/merge drivers, credentials, SSH
commands, and other local authority fail before `status` or `ls-files`. The
entire bounded `.git` inventory is also hashed with non-following traversal;
`commondir`, linked-worktree metadata, alternates, grafts, replace refs,
shallow state, nested links/reparses, hard links, and special files fail
closed. Exact Git-dir, common-dir, objects, refs, and work-tree paths must
remain inside the disposable root. System attributes and replace objects are
disabled explicitly, while local `info/exclude` and `info/attributes` bytes
remain in the repository-authority digest.

The same Git binding now crosses the Bridge boundary for writable elevation.
Every new `verify-mutation` requires path/size/SHA as one all-or-none tuple,
scopes it over every nested Git `Popen`, and persists the canonical executable
record in task-local CAS. Exact committed v2 replay recovers the binding from
that CAS object before any endpoint recapture or Git subprocess. A new
elevation with no tuple fails before Git can start. New mutation evidence uses
`codex_mutation_verification_v2` and references that CAS object; the v1 reader
remains for already-written evidence only when the caller supplies an active
exact Git executable binding; the legacy projection deliberately reports no
historical Git executable identity. New Bridge endpoint capture writes
`aoi.codex-transport.git-mutation-snapshot.v3` and binds a closed
repository-observation-authority digest across both endpoints. The reader
retains snapshot v2 for existing records, while generic AOI integrity also
produces v3. A canonical linked worktree v3 binds its `.git` file,
per-worktree Git directory, `commondir` and back-reference, common
objects/refs/config, and per-worktree HEAD/index/refs. Executable or
per-worktree config, split indexes, non-canonical aliases, metadata-routing
drift, and alternate/grafted/replacement/shallow history fail closed. Bridge
elevation remains explicitly standalone-only. Portable v2 cannot elevate a new Bridge mutation;
mixed endpoint snapshot versions and authority drift cannot elevate new evidence. The provenance scope is
intentionally `bridge_verify_mutation_git_observation`: it binds Bridge
validation, recapture, publication, and replay observations but does not
retroactively prove which Git binary created an older pre-endpoint.
GitHub Actions OIDC, runtime, results, and cache authority variables join the
shared publication-credential scrub. Workload enumeration uses bounded,
sorted `scandir` traversal, rejects
symlinks, Windows reparse points/junctions, hard-linked files, special files,
and directory/file identity drift before reading bytes. Old v2 specs are
unsupported and must be regenerated from reviewed authority.

Direct verified-mutation capture accepts only an ordinary standalone
`root/.git` repository. It rejects linked/common worktrees, gitlinks,
alternates, grafts, shallow state, replacement refs, and local
include/filter/fsmonitor/external-attribute authority. The closed observation
environment is used for both endpoints, including the pre-endpoint whose
historical executable provenance cannot be inferred. The writable canary
retains canonical direct pre/post workload manifests and requires every direct
delta path to be present in the committed post-mutation path set. A no-op,
ignored-only mutation, tracked-plus-ignored mixed mutation, or metadata-only
repository-authority mutation fails closed. The core elevation also requires a
nonempty post mutation path set and a changed file/tree/status projection that
excludes commit identity; an empty commit or unchanged dirty-path state under a
same-tree HEAD change cannot become `verified_mutation`. Strict JSON rejects
duplicate keys and non-finite numbers, and subprocess stdout/stderr limits are
enforced during streaming execution.

The standalone restriction is an explicit Bridge endpoint option, not a global
AOI Git ban. The first commit-bound integrity attempt on this successor exposed
that applying it to generic `task_mutation_snapshot()` made the legitimate
release linked worktree unsealable. A rejected first repair tried to restore
linked-worktree support by emitting portable v2, but fresh review found that it
both skipped repository config authority and broke byte-identical reproduction
of existing v3 integrity artifacts. Generic integrity now stays on v3 and
binds a canonical linked worktree's per-worktree and common authority; Bridge
capture explicitly selects the standalone layout, and materialization still
rejects any v2 endpoint. The failed integrity attempt published no record or
seal; commit `8629787c477c4fa326eefef7bd2d7f07eff052d5` is therefore
predecessor evidence after this repair.

Fresh repair checks on the current uncommitted bytes are deliberately narrow:

- `tests/test_codex_transport_canary.py`: `62 passed, 2 skipped`;
- `tests/test_git_plumbing.py`: `73 passed, 45 subtests passed`;
- `tests/test_codex_transport_contracts.py` plus
  `tests/test_codex_transport_mutation.py`: `91 passed, 1 skipped`;
- integrity command modules, the prior nine focused CLI integrity cases, and
  a real linked-worktree candidate/review/seal artifact replay pass
  sequentially with `32 passed, 9 subtests passed`; the replay asserts
  persisted/current canonical bytes, authority digest, runtime integrity, and
  task doctor;
- predecessor `8629787` and the repaired implementation emit byte-identical
  standalone-v3 snapshot JSON (`529` bytes) from the same clean standalone
  scratch clone, preserving existing v3 artifact replay;
- `tests/test_codex_app_server_stdio.py` plus
  `tests/test_confidentiality.py`: `141 passed, 3 skipped`;
- an attempted six-node `tests/test_codex_transport_cli.py` shard reached its
  124-second orchestration timeout without a pytest verdict. This is a typed
  procedural failure, not a test failure or a passing result; the complete
  40-case file remains pending for exact-clean-commit qualification;
- targeted mypy passes for six Git/Bridge/canary/confidentiality modules, all
  221 tracked Python files parse from bytes, and `git diff --check` passes.

Fresh independent dirty-byte review accepted result
`fa1ed6302ca9e4a42c9211dec461ebc3acf635c4cd86f6a5278b592df9bbcfe9`
with P0/P1/P2 = `0/0/0`. The reviewer independently reproduced the combined
Git-plumbing/mutation result (`92 passed, 1 skipped, 45 subtests`), the linked
integrity replay (`1 passed`), and a clean diff check. This verdict authorizes
forming a clean candidate; it is not the required exact-commit integrity
review or seal.

These are dirty-byte implementation, test, and review checks only. They are not
a clean commit, package/install result, full Windows or fresh-ext4 WSL result,
live App Server canary, Git mutation verification, exact-commit integrity
review/seal, remote CI, or publication evidence.

The executable and config checks are cooperative observations, not an atomic
hostile-writer lock: the file handle is closed before process creation and a
same-account writer could race the verify-to-spawn boundary. They bind the
declared executable files, not the OS loader's executed image, DLLs, dynamic
dependencies, or code-signing trust. Live acceptance must preserve this
evidence boundary.

Evidence remains separated:

1. **Contract tests** are the fresh dirty-byte focused/static results above
   plus the still
   required final-byte full Windows, fresh-ext4 WSL, and subprocess-aware
   coverage runs.
2. **Live App Server canary** is still absent on this successor. The exact
   pinned 0.145.0 executable/schema must pass read-only scratch first. A live
   acceptance run must use a trusted protocol-v6 `SessionStart` hook receipt;
   a preparation script may not directly synthesize `source=startup` authority.
   The canary v4 spec additionally binds the exact isolated `CODEX_HOME`,
   Bridge bytes, and Git executable bytes; every subprocess revalidates the
   relevant executable binding. It overrides ambient `CODEX_HOME`, closes Git
   routing/config authority, and scrubs AOI authority, known publication/OIDC
   credentials, SSH control variables, and Python path/home/venv influence.
   It disables the user site and unsafe path insertion while retaining
   model-service authentication. The trusted SessionStart receipt and final
   canary evidence must additionally bind the installed
   wheel/RECORD/runtime provenance. The standalone driver validates that
   closure independently rather than importing the package under test.
   Rejected v1/v2/v3 preparation assets are
   diagnostic history only and must not be resumed.
3. **Git mutation verification** is still absent. It requires a separate
   disposable writable scratch, authenticated typed claim coverage, and exact
   pre/post Git endpoints after the read-only live pass.
4. **Remote CI/publication** is still absent. GitHub `test.yml` and main-only
   `docs.yml` must pass on the exact final pushed SHA before tag, GitHub
   prerelease, PyPI, promotion, or ARISE installation.

### 2026-07-24 exact-gate falsification after `35a1beef`

The exact Windows coverage run terminated with `16 failed, 2218 passed,
26 skipped, 487 subtests passed`; fail-fast therefore produced no combined
coverage report or source-after receipt. The exact fresh-ext4 WSL run
terminated with `29 failed, 2132 passed, 39 skipped`. Both runs are failed
historical diagnostics, never promotion PASSes.

The failures exposed four independent boundaries:

- a standard Linux linked worktree may have no lazily created per-worktree
  `refs/` directory; generic integrity must bind that absence distinctly while
  still rejecting linked/hard-linked contents when it exists;
- the Permit CLI legacy fixture retained its test-only structured claim across
  semantic migration, which the production quiescence gate correctly rejects;
- the exact-test runner resolved a WSL venv launcher symlink to base Python and
  lost the venv pytest installation; child execution must preserve
  `sys.executable` spelling while the receipt may still hash the resolved
  interpreter image;
- native Windows directory metadata identity alone does not reliably expose a
  transient create/delete that reverts before inventory completion. The
  proposed watcher repair is not accepted until overlapped I/O is cancelled,
  drained, and independently reviewed without releasing its buffer or
  `OVERLAPPED` early.

Focused repairs have local test evidence only. All exact Windows/WSL,
coverage, package/install, live-canary, review, integrity, and publication
gates must restart from a new clean successor commit after the final watcher
repair.

### 2026-07-24 current dirty-byte canary and cross-platform repair

The Windows watcher repair is now bounded by an armed
`ReadDirectoryChangesW` request before baseline capture and an exact
`CancelIoEx` plus blocking `GetOverlappedResult` drain after the final
identity. Buffer, `OVERLAPPED`, event, and directory-handle ownership remain
live until that drain. A successful cancellation completed with
`ERROR_OPERATION_ABORTED` is the finite observation boundary; mutations after
that boundary are outside the claim. Capture and verify each have a native
transient create/delete falsification, and disabling the watcher makes both
tests fail. A fresh reviewer accepted the repaired watcher with P0/P1
`0/0`; its P2 handle-ownership cleanup finding was then repaired so a failed
`CloseHandle` leaves ownership available for explicit retry. The current
watcher/release slice passes `69 passed, 2 skipped, 2 subtests`, strict mypy,
compileall, and diff check. This is local dirty-byte evidence, not a final
Windows inventory receipt.

The first independent review of canary v4 correctly rejected two P1 gaps: a
self-consistent installed `RECORD` could replace an installed module without
being compared with the wheel, and the child inherited ambient runtime and
loader variables through a denylist. The successor repair now reads one
bounded wheel payload and checks the SHA-256 of those exact parsed bytes
against both the spec and package receipt. It validates the ZIP member set,
wheel `RECORD`, every member hash/size, and `Root-Is-Purelib`, then compares
every installed wheel-owned AOI package/dist-info byte with that exact payload.
Installer-created `INSTALLER`, `REQUESTED`, PEP 610 `direct_url`, installed
`RECORD`, and all four exact AOI console launchers are a separate closed
surface derived from the wheel's exact `entry_points.txt`; the declared Bridge
launcher must equal the spec-pinned executable. A second independent review
then correctly rejected direct execution of that installer-generated launcher:
a malicious launcher plus a self-consistently rewritten installed `RECORD` and
spec pin still had no wheel-derived execution semantics. The repaired driver
therefore never executes a pip launcher. It binds the base Python already
trusted to run the canary, revalidates its exact path/size/SHA-256, and invokes
that interpreter with `-I -S -B -X utf8` plus a fixed bootstrap for only
`aoi_orgware.codex_transport_cli` from the exact site-packages root. The
bootstrap appends that root after the isolated trusted standard-library paths;
an unrecorded top-level module therefore cannot shadow a standard-library
import. Regressions prove that a deliberately non-executable fake launcher is
not invoked and that a hostile installed `argparse.py` cannot execute.
Top-level `.pth`, AOI import shadows, and any `sitecustomize`,
`usercustomize`, or `site` source, bytecode, extension, or package spelling
fail closed. Empty wheel members such as `py.typed` remain valid and are still
byte-compared.

The same review found that a substring check for
`include-system-site-packages = false` accepted a duplicate trailing `true`
and hostile `home`. The repaired parser rejects duplicate or unknown keys,
requires one exact false value, and binds `home`, `executable`, full Python
version, the exact terminal creation prefix, and the trusted base interpreter;
a prefix-superstring is rejected. The venv launcher and `pyvenv.cfg` remain
installation observations; execution uses the separate trusted base-Python
binding and disables `site` processing.

Bridge subprocesses now receive an explicit constructed environment rather
than a copied-and-scrubbed ambient environment. It contains only the exact
isolated homes and executable directories, Python isolation flags, locale,
validated temp directory, closed Git controls, and on Windows a native
`GetWindowsDirectoryW`-derived system root and command processor. Ambient
Python/Git/SSH/publish/OIDC/OpenAI/proxy/loader/Node variables and unknown
names are not forwarded. This remains a cooperative process-launch
observation, not an OS-loader, DLL, kernel, same-user, or model-provider
attestation.

Current exact-slice evidence is deliberately separated:

- **Contract tests:** the current canary bytes pass
  `100 passed, 2 skipped` on native Windows and independently
  `100 passed, 2 skipped` under WSL Python 3.12. The WSL run first exposed a
  test-only `core.filemode=true` no-op; the regression now toggles the observed
  value and passes on both platforms. Transport CLI `42 passed` in 339
  seconds, including already-expired and post-issuance semantic-head drift
  preflights that preserve `permit_consumed=false` and the armed packet;
  strict mypy passed the canary and Bridge CLI source, and compileall/diff
  checks pass. Earlier disjoint current-repair slices passed permit/migration
  `31 passed, 21 subtests`, exact receipts `31 passed, 1 skipped`, linked
  integrity `75 passed, 45 subtests`, broad Transport
  `255 passed, 2 skipped, 6 subtests`, and watcher/release
  `69 passed, 2 skipped, 2 subtests`.
- **Exact-wheel install structure:** a disposable native Windows venv installed
  the exact predecessor package-gate wheel with `pip --no-index --no-deps
  --no-compile`; the repaired driver accepted 125 AOI namespace files, 129
  installed `RECORD` rows, and the exact four launcher names. This validates
  real pip layout compatibility only. It does not transfer the predecessor
  package receipt, source identity, or PASS to the pending successor.
- **Live App Server canary:** absent on current bytes. Read-only must precede
  writable, and both require newly issued exact permits and current package
  closure.
- **Git mutation verification:** absent on current bytes and requires the
  separate disposable writable run after read-only live acceptance.
- **Remote CI/publication:** absent. No current successor commit, package,
  integrity review/seal, push, tag, GitHub Release, PyPI publication, promotion
  bundle, or ARISE installation exists yet.

The second independent rereview is preserved as rejected result
`a14578294eb4794ca17fe38d9d7fdf813cac07259790be9a4430709c8b537550`
with P0/P1 `0/2`: direct installer-launcher execution and ambiguous
`pyvenv.cfg`. The third independent rereview is separately preserved as
rejected result
`ebd7d4b8d22b9ecd6d9155fad39448996f9cd8fbc35811e7ec8e0b8b9f0cfb97`
with P0/P1/P2 `0/1/1`: installed-site precedence over the standard library and
a prefix-superstring accepted by the creation-command check. All four findings
now have the repairs and focused regressions described above.

Fresh independent rereview v4 accepted the repaired canary with P0/P1/P2
`0/0/0`; the immutable AOI result is
`bedc5e50513e4ef5d4fa8648526d1bac1d98d1fae387cffca524e24ea6b61bfa`.
It independently ran a 30-test security subset and the full canary file
(`100 passed, 2 skipped`) while binding the reviewed implementation and test
files to SHA-256
`17e45dc80218ecff1b1e11eab2f5dd872b83a24922eb59e78f58e0b10d2849a6`
and
`df1c6a255af7b70bf6b1b7ef5a26d219d5be6a2bbd4dc6dc59db698388096da9`.
This dirty-byte review authorizes only the next clean-candidate and full-gate
phase; it does not authorize publication.

## 2026-07-24 P0 semantic-v2 claim lifecycle release gate

Before v0.4 release, semantic-v2 claims must use only typed
`claim_acquired`, `claim_status_changed`, and `claim_released` lifecycle
events. Acquisition binds one immutable claim object to one immutable
task/head binding; projections and free-form checkpoints never replace that
authority. Each status or release transition publishes a new immutable
revision object and binding chained through `prior_object_sha256`; it never
rewrites an earlier claim object. A `pending` reservation reserves scope only:
it cannot authorize a Bridge launch or mutation. Every status writer uses the
exact expected-head CAS, and an idempotent retry is limited to the same command,
expected head, object, binding, and result.

Release is ordered: append `claim_released`, publish the terminal immutable
archive, then unlink the active entry. Migration must reject any structured
legacy active or archive claim until an explicit typed import has produced the
corresponding object, binding, and events. Doctor, close, and every Bridge
authority decision must use an authenticated semantic loader that replays and
validates ledger, object/binding identities, and typed current status; missing,
tampered, legacy-unimported, or projection-only authority fails closed.

Claim transitions advance the task revision, so this release gate also includes
a deterministic `task_checkpointed` semantic transition. It binds the supplied
timestamp, resulting revision, checkpoint revision, and checkpoint SHA-256;
the derived `checkpoint.md` is written only after the event. Without this port,
a correctly released semantic claim would still leave close permanently
blocked by a stale checkpoint.

This P0 is required before release, but its evidence is deliberately separate:
claim contract/falsification tests establish the lifecycle contract; a newly
issued live App Server canary establishes transport behavior; exact before/after
Git mutation verification establishes mutation evidence; and authenticated
remote CI establishes only the final-SHA CI gate. None substitutes for another
or for release/promotion evidence.

## 2026-07-24 checkpoint liveness repair

The long-lived ARISE K vertical-slice task exposed a control-plane liveness
defect before the resource-config compatibility checkpoint commit. Revision
962 produced a 34,084-byte existing compact projection while the prior
revision-957 checkpoint was already 32,626 bytes. The failed checkpoint attempt
changed neither state nor the previous checkpoint, as required, but the runtime
had no general recovery projection. Truthfully releasing compatibility-only
claims later reduced revision 969 to 31,715 bytes; that was an operational
workaround, not a fix.

The isolated v0.4 repair keeps the 32 KiB fail-closed boundary. Full rendering
and the existing terminal-history projection remain byte-compatible except
that every engaged lane is now retained instead of silently truncating the
terminal form after four lanes. Only when both forms still exceed the ceiling
may a second renderer project append-only facts, decisions, and rejected paths
as count, ordered `json-list-utf8-v1` SHA-256, exact state-field reference, and
a byte-bounded tail of complete entries. `state.json` and any semantic ledger
remain complete. Claims/locks, active jobs/packets, pending
verification, engaged lanes, open incidents, blockers, open risks, changed
files, delivery, and exact next action remain verbatim; an oversized
marker-only active view still fails without publication. Optional recent-tail
budgets shrink before required detail can cross the ceiling, and `doctor`
verifies projected markers and tails through a fixed renderer-owned machine
block rather than scanning free-form Markdown. The ordinary checkpoint
transaction is the recovery command, and its existing
checkpoint-before-state crash mismatch remains exactly retryable.

This repair is being qualified independently from resource-config
compatibility, Windows path canonicalization, coverage-path provenance,
release-tag/main race hardening, live App Server canaries, and publication. It
does not authorize ARISE RTL/EDA work, a tag, GitHub Release, or PyPI upload.

## 2026-07-23 selective publication closure repair

**Superseded historical checkpoint — 2026-07-23 remote publication update:**
`v0.4.0a1` and `v0.4.0a2` were
...
ARISE install may use `v0.4.0a1` or `v0.4.0a2`. At that historical checkpoint,
the publication candidate was retargeted to `0.4.0a3`. It is now superseded by
the active `0.4.0a4` release-successor section above; no `v0.4.0a3` publication
...
The v127 independent dirty-byte review rejected promotion with three P1 and
two P2 findings: fetch URL readback could be confused with a distinct push
URL; the Chief-free handlers did not reload locked configuration or hash the
actual approved plan; the runbook pushed a mutable tag ref without immediate
mechanical revalidation; the delivery receipt omitted the exact preflight
task-CAS edge; and its public validator trusted an unvalidated preflight
mapping. The v128 successor addresses those findings without broadening the
cooperative boundary. Preflight remote-state inspection and delivery readback
now use the exact effective push transport, config/plan/task/HEAD/tag/evidence
are reloaded around network observations, an exact object/create-only-lease
procedure replaces mutable-ref push, and delivery validation requires the
exact preflight verification index, record SHA-256, artifact SHA-256, receipt
SHA-256, and fully revalidated exact-CI-bound preflight.

A pre-formal v128 audit then found two remaining P1s and one P2 test gap:
the runbook did not fail explicitly when reproduced preflight bytes or their
CAS artifact SHA differed; local and workflow tag checks accepted a tag whose
embedded name differed from its ref or whose direct target was another tag;
and verify-side config/plan/tag drift lacked fault injection. The v129
successor rehashes both receipts and fails before push on any mismatch, extracts
the exact push inputs only from the verified recheck, requires the local and
workflow tag header to directly name the expected commit/ref, independently
checks the GitHub tag object's name/direct commit before every publication
boundary, and adds verify-side drift tests.
...
- targeted release-tag matrix: `109 passed, 2 subtests passed`, artifact
  SHA-256
  `931ef80a342310e298f7f3fe2d3f3b48e94a943ae7d5e62b05ffe304149bcfbe`;
- expanded focused matrix: `220 passed, 8 skipped, 2 subtests passed`,
  artifact SHA-256
  `31c1230cb449fb248114d910ab56791917805655b1ae865fe6c6d28dd5637ae2`;
- strict native and Win32 mypy for the six release-route source modules,
  compileall, and `git diff --check`: pass.

The source, test, workflow, runbook, and documentation bytes immediately before
this evidence paragraph are what those two pytest artifacts cover; the
containing evidence-only documentation update requires its separate contract
rerun. These remain focused dirty-byte evidence. Formal AOI packet review,
full Windows/fresh-ext4 WSL qualification, clean candidate, integrity-v2 seal,
tag delivery, remote CI, GitHub/PyPI publication, promotion bundle, and ARISE
...
- targeted release-tag matrix: `112 passed, 2 subtests passed`, artifact
  SHA-256
  `d5cbcb2de77484ff99195fbc45fbea928939af394be631a4cec7fad58868c113`;
- expanded focused matrix: `224 passed, 8 skipped, 2 subtests passed`,
  artifact SHA-256
  `6f0c2b28efbd2ab938e2e21ee895aca676a6f1403266aa634d0a682d9c091eab`;
- native and emulated-win32 mypy: no issues in seven release/publication
  source files; compileall and `git diff --check`: pass.
...
- the reconstructed fresh tree had no `HEAD`, so a test that archives `HEAD`
  correctly failed;
- pytest was borrowed from a venv outside the fresh checkout, violating the
  new dedicated-runtime provenance contract and invalidating three hook/
  onboarding fixtures;
- the ordinary workflow contract still required the superseded 45-minute unit
  timeout even though the canonical plan, changelog, workflow, and release
  contract require 90 minutes (coverage remains 120).
...
These results establish local Windows and native WSL runtime qualification for
the frozen dirty source tree only. They do not establish independent review,
integrity-v2 sealing, package/install acceptance, remote CI, tag creation,
publication, promotion, or ARISE installation. The containing documentation
contract, task-CAS evidence records, and later exact clean-candidate gates
remain required.
...
- The optional `aoi-codex-bridge` entry point exposes finite `issue`,
  `preflight`, `run`, `inspect`, and `verify-mutation` commands. Only `issue`
  accepts a Chief credential. `preflight` authenticates one still-issued
  permit/prompt/packet/pre-Git boundary without consuming or publishing;
  `run` repeats that validation before the single reservation. Neither can
  receive or retain reusable Chief authority.
...
### 2026-07-24 exact-successor qualification reset

Exact candidate `559b852b43b62976a51d969e348e6c0efc01f2ba` is rejected
qualification evidence. Its fresh-ext4 WSL run terminated with `2 failed,
2200 passed, 46 skipped`: initialization detected that the packaged policy was
older than canonical `docs/POLICY.md`, and a full-suite legacy
`verification-supersede` sequence observed the host/WSL clock earlier than its
replacement record. The second failure passes in isolation, but it exposed a
production invariant rather than an acceptable flaky test: AOI could publish
`superseded_at` earlier than the replacement it causally names.

Exact child `77a66332ef1f6a9914d64d0904a879f81713781d` synchronizes the
packaged policy bytes and passed its exact package/install gate. It is
nevertheless also rejected for promotion because the independent clock-order
defect remains in those bytes. Its package review additionally found that the
release verifier checked policy presence but did not bind its bytes to the
canonical source policy.

The next successor therefore makes two bounded repairs before any new
candidate is frozen:

- legacy supersession uses the current clock only when it is strictly later
  than the replacement; rollback of at most five seconds is represented as
  replacement plus one microsecond, while a larger rollback fails before task
  state mutation. Semantic-v2 continues to use its authenticated semantic
  timestamp unchanged;
- `verify_dist.py` now requires exact canonical policy bytes in the wheel,
  original sdist, and wheel rebuilt from that sdist, with unique-member,
  regular-file, bounded-size, and complete-read checks.

The dirty four-file repair has focused Windows evidence of `25 passed` plus
two subtests, fresh-ext4 WSL evidence of `25 passed`, target mypy/compile and
diff checks, and a successful exercise of the strengthened verifier against
the historical `77a6633` artifacts. Its first correctness review rejected
prefixed and symlink-typed derived-wheel policy members plus two adjacent
timestamp/read-bound advisories. After repair, the immutable-snapshot
re-review returned P0/P1/P2 `0/0/0`; collaboration routing remains truthfully
`manual_unverified` because no authenticated `SubagentStart` observation was
available. These are implementation-slice results, not evidence for a new
package or release. A clean successor, fresh package, complete
Windows/coverage/WSL gates, whole-candidate independent review, live canaries,
integrity-v2 seal, remote CI, publication/readback, promotion, and ARISE
...
4. **Remote CI:** required after the sealed exact commit is pushed for this AOI
   ...
   `test.yml`/`docs.yml` observations must pass the offline validator and be
   recorded in task CAS before the release-tag composite preflight. The publish
   workflow independently repeats the exact correlation before its first
   GitHub Release mutation. Historical runs, a tag-only test, or a receipt from
   another commit/destination do not qualify the containing candidate.
...
- **O8 v2 implementation candidate; promotion gates remain:** the historical
  ...
  superseded; the final matrix includes local Windows/fresh-ext4 WSL and pushed
  exact-final-SHA GitHub Linux/Windows plus docs gates. The required-v2
...
12. `item/completed` and `turn/completed` are
    ...
    current claim coverage, source/tree bindings, and Bridge-side exact-Git
    observation provenance can elevate a new v2 mutation record. That Git
    observation is cooperative verify-before-spawn evidence, not atomic loader
    provenance or retroactive proof of the original pre-endpoint capture.
...
name, version, and allowed artifact hashes of all 40 locked distributions
(56 wheel hashes total). Build the
...
### 2026-07-24 clean-successor transport contract

The first transport journal format is frozen as a read-only compatibility
format. Its published intent, reservation, ordered journal, terminal receipt,
projection-row, and published-row hashes are fixed by golden tests. A newly
issued launch must use transport contract v2 from `reserved` through its
terminal receipt; one launch journal may not mix v1 and v2. The task namespace
may still project historical v1 rows beside new v2 rows.

V2 calls the initial evidence `transport_reserved`. Before any request bytes
are written to App Server stdin, a durable request witness binds the immutable
intent and permit, expected semantic head, exact canonical JSON-RPC request
bytes and size, prompt digest and size, cwd/model/effort, approval/sandbox,
`networkAccess=false`, exact runtime/schema pin, closed `thread/start` config,
and current thread/turn correlation. Failure to durably persist that witness
means no request write. Loss after a start request may have been written keeps
that same witness in `launch_unknown`; automatic resend remains prohibited.

Single-launch and aggregate inspection now use the same authenticated
lifecycle formatter. They expose committed versus pending records, exact
intent/reservation/issuance/binding references, request witnesses, terminal
receipts, and the evidence level. `reserved` is not runtime evidence;
`item/completed` and `turn/completed` are only
`codex_runtime_observed`. A later mutation elevation must independently bind
the supplied post-Git endpoint; replay with different Post B bytes is rejected
even when Post A and the transport receipt still match.

The disposable canary driver is opt-in: its default calls the Bridge
`preflight` surface and accepts only `issued_unconsumed`,
`permit_consumed=false`, an armed packet, and no runtime evidence.
`--execute` then repeats the same authenticated checks inside `run` before the
one transition that reserves/consumes the permit, followed by terminal
`inspect`. The driver never treats `transport_reserved` as a dry-run result.
Its v4 input also requires the independently checked installed-distribution
closure described above. It requires an exact runtime pin, a committed
marker-bearing scratch repository with no remote or URL rewrite, a bounded
self-contained Git metadata inventory, and rejects an `ARISE` root.
Repository config, common-dir/object/ref routing, alternates, grafts,
replacement refs, shallow state, system attributes, nested links/reparses,
and executable/package provenance all fail closed before evidence is
accepted. The read-only phase requires an identical pre/post workload
snapshot; the writable phase requires a real workload delta plus separate
`verified_mutation` elevation whose v2 semantic object references the exact
Bridge-side Git binding. Structural tests use a fake bridge except for the
real local Git plumbing and CLI mutation tests. No live App Server canary has
been run on these successor bytes, and none is authorized until a new clean
commit, exact package receipt, and stable runtime/schema pin are requalified.

The former writable-live dependency on task-local semantic-v2 `claim` and
`release-claim` transitions is closed in the containing successor. Typed claim
objects/bindings/events, authenticated consumers, archive-before-unlink, exact
retry, migration fencing, and deterministic checkpoint reachability pass the
integrated focused tests. This removes a contract blocker only. A writable
canary is not authorized until the exact final candidate first completes a
fresh read-only live pass and then receives its own disposable-root claim,
packet, arm, and one-shot launch permit.

That requalification found a reproducibility defect in the historical raw-byte
schema pin: two exact 0.145.0 generator runs emitted equal JSON schemas with
different object-key ordering and therefore different raw hashes. V1 must keep
its historical raw binding so old content-addressed records remain readable.
New V2 launches require a separate, strict-JSON, duplicate-rejecting canonical
semantic pin over every generated file and the combined v2 schema. The two
independent 273-file outputs now agree at canonical manifest
`c05875501c6e9a6778cc4afc5488cdb87aae539217121ebbb5c8dd14c79bc025`
(36,091 bytes) and combined-v2
`27f8d983f19d8e1a5548d52176de0a460fb05aaf2a72110f913c6f4af2bd4f27`
(269,688 bytes). Package and adapter readers use only that V2 identity, while
the old raw `6b8b...`/`6253...` tuple remains a V1 reader constant. Duplicate
keys, non-finite JSON, noncanonical package bytes, and semantic divergence
between repeated generator outputs all fail closed. Schema-refresh input roots
are rejected before resolution when they are links or carry any native Windows
reparse attribute; linked/reparse descendants are rejected as well. Native
Windows canary policy comparison uses the same slash-canonical absolute paths
authenticated by V2 contracts. This is contract and package evidence only;
fresh independent review, package/install validation, and live canaries remain
separate gates.
...
3. **Git mutation verification:** AOI before/after snapshots, exact endpoint
   claim coverage, source/tree binding, exact Bridge-side Git executable
   provenance, and a separately recorded elevation from
   `codex_runtime_observed` to `verified_mutation`. Endpoint coverage is not a
   claim of continuously held authority without a claim-history receipt, and
   the exact Git verifier does not retroactively identify the binary used to
   create an older pre-endpoint.
4. **Promotion environment:** exact final-SHA GitHub Linux/Windows test and
   main-only docs gates are available when no protected rule is exposed, or
   when every exposed subject is allowed at its exact home repository. Local
   fresh-ext4 WSL is separate required WSL evidence. This AOI release has no
   protected rules and requires those remote gates after exact local
...
### 2026-07-25 exact-test and release-tool successor boundary

The exact `ec6d6aecddb09169276f7d37d4db29144092ce02` candidate is rejected and
must never be promoted, pushed, tagged, published, or installed. Its independent
whole-tree review found two release-gate defects: the exact-test producer
accepted caller-controlled pytest collection surfaces, and `test.yml` supplied
Twine from an ambient online install outside the hash-locked release toolchain.
The candidate's package, partial Windows/WSL, and earlier review evidence remain
historical and cannot be rebound to successor bytes.

The successor exact-test producer uses runner version 3 while retaining the
explicit `pytest-contained-argv-v2` argument contract. Version 2 introduced
the contained argv/config/environment contract: new evidence records both
requested and canonical effective argv and binds a runner-generated empty
pytest config, snapshot root/confcutdir/cwd roles, environment digest,
plugin-autoload disablement, and `PYTHONNOUSERSITE=1`. Version 3 adds
same-explicit-environment WSL platform capture and pre-launch
distro/kernel/domain correlation. The caller grammar is limited to `-q`, one
enumerated `--tb=...`, contained `--ignore=...` paths, and exact-spelling
targets below `tests/`. Response files, arbitrary options, path aliases,
absolute/drive/UNC/device paths, missing targets, links/reparses, and external
collection fail before pytest starts. Historical version-1 and version-2
receipts remain parseable by integrity/doctor replay, but every new
`add-verification` exact evidence ingestion requires the current version-3
producer.

The release lock now contains 40 distributions and 56 allowed wheel hashes,
including `twine==6.2.0` and all selected transitive dependencies for the
Windows/Linux Python 3.11--3.14 release runners. That uniform closure includes
keyring's Linux SecretStorage, jeepney, cryptography, cffi, and pycparser
dependencies, plus the Python 3.11-only backports.tarfile,
importlib-metadata, and zipp marker closure. `test.yml` and every
`publish.yml` producer/rebuild path download with `--require-hashes`, install
offline, and invoke Twine or build tools only through that environment.
`release_rehearsal.py release-toolchain` parses dotted/underscored names and
multi-hash requirements, verifies the installed version of every distribution,
and emits the canonical lock SHA/name/version/complete-allowed-hash-set
binding; duplicated single-hash parsers and hard-coded workflow maps are
removed.

Current dirty-successor evidence includes focused Windows contract tests:
exact-test/CLI-CAS is 85 passed, 1 skipped, 19 subtests; release
rehearsal/workflow contracts are 44 passed, 1 skipped. Targeted mypy,
compileall, and `git diff --check` pass. Real CPython 3.11 fresh Windows and
WSL probes each resolved 40 wheels, installed offline with hashes required,
passed `pip check`, and emitted the same release-toolchain receipt SHA-256
`24e3e6807e1294919bc8fcd983c2855df1003e39790c51d1d67c36d075e634f1`
for lock SHA-256
`534c4909cd9bcbd249866e3e826adae7fd80827727cc3180cbf37a965a4afcd5`.
This is still dirty-byte evidence, not exact-commit evidence. A clean successor
commit, rebuilt package/install evidence, exact
Windows/coverage/fresh-ext4 WSL suites, live App Server canaries, integrity-v2
seal, doctor, exact-main remote test/docs, publication readback, promotion, and
ARISE installation remain mandatory.

The first clean successor,
`8de626a7b6a1e08c9c71efc4b7136e0c87c5ae57`, is also rejected. Its local
Windows package build passed, but a fresh Ubuntu WSL resolution failed before
tests because the 32/40 lock omitted keyring's Linux-only SecretStorage chain.
That package and its review/test outputs are diagnostic only and cannot be
promoted or rebound. The 40/56 closure above is the successor repair. Its dirty
bytes passed fresh Python 3.11 Windows/Linux resolution and independent review;
every exact-candidate gate must still be rerun after the clean commit.

### 2026-07-25 `fd5984b` fresh-ext4 WSL rejection

The next exact candidate,
`fd5984b51a4ae041eb582657f6f8b979b833dc98`, is also rejected and must not
be promoted, pushed, tagged, published, or installed. Its locked
Hatchling 1.27.0 rebuild reproduced the candidate wheel and sdist, and its
reviewed local-install bundle passed, but those results are historical after
the canonical fresh-ext4 WSL full suite rejected the candidate. No package,
review, install, or partial Windows evidence from this commit may be rebound to
successor bytes.

The exact WSL receipt recorded `accepted=false`, exit 1, unchanged source
identity, `21 failed, 2278 passed, 46 skipped, 483 subtests passed`, receipt
self-digest
`ebebf6eaad35bc68d62d12fb3758905cb6f4a86f4a3de36a64afdfd9f17f6a14`,
raw-file SHA-256
`ffa4c88c458ae51fe84a48ca479170b375a47be570c32e58ef73e53b31a31efc`,
and log SHA-256
`a8378c423c3aa1b4be1e245a238625e71a369f3dd824860a43e7fdd03175b30f`.
The failures have three bounded causes:

- the contained exact-test child dropped `WSL_DISTRO_NAME` and `WSL_INTEROP`
  while retaining a Microsoft kernel, so the production onboarding policy
  correctly rejected 19 doctor/onboarding/offboard tests as partial WSL host
  signals;
- one legacy resource-config test constructed two orphan/forged fixtures with
  unconstrained wall-clock `now_iso()` instead of timestamps strictly after
  the recorded rollback, so a clock step could reject the test before its
  intended cross-binding assertions; and
- one publication workflow test called `git archive HEAD` inside the exact
  runner's intentionally Git-metadata-free blob snapshot.

The successor runner-v3 repair preserves both WSL routing variables in the
closed child environment, captures the platform claim from that same explicit
mapping, and binds their coherent domain/distro/kernel identity in the current
receipt protocol; it does not weaken the production partial-signal rejection.
Historical runner-v1/v2 receipts retain read compatibility but cannot be
ingested as new current evidence. The resource fixture derives migration time
strictly after rollback without changing the production validator. The
publication test retains its stronger `git archive` path in ordinary checkouts,
rejects lexical local/ancestor Git metadata before using the fallback, and uses
only exact product source plus the reviewed policy snapshot inside the
intentionally gitless test fixture; source provenance remains the outer exact
receipt's responsibility. These are successor dirty-byte repairs only. A new
clean commit must rebuild package/install evidence and rerun Windows full,
subprocess-aware coverage, fresh-ext4 WSL full, independent review, both live
canaries, integrity-v2 seal, doctor, exact-main remote CI,
publication/readback, promotion, and ARISE installation.

### 2026-07-25 `bcd3dcb` exact-commit review rejection

The clean successor
`bcd3dcb16fff37485e0023657ced6c5b2745385d`, tree
`e5af1761d3d91313f49139f23a00138c492e72eb`, is also rejected and must
not be promoted, pushed, tagged, published, or installed. Its fresh
exact-commit review reported P0/P1/P2 = 0/0/1: the runner-v3 repair added
`WSL_DISTRO_NAME` and `WSL_INTEROP` to the shared receipt environment
allowlist, so a newly resealed runner-v1 or runner-v2 receipt could claim
environment names that those historical producers could not emit. Current
evidence ingestion still required runner v3, but integrity and doctor replay
would have accepted an unnecessarily expanded historical language.

The successor must select the environment allowlist by producer version.
Authentic v1/v2 receipts without the two WSL names remain byte-readable;
resealed v1/v2 receipts carrying either v3-only name fail closed. Direct
negative tests cover both historical producer versions and both names. This
narrows historical replay only; it does not change runner-v3 WSL capture,
current-protocol ingestion, or onboarding's partial-signal rejection.

A locked `bcd3dcb` wheel and sdist were built before the review completed, but
they and their partial package checks are diagnostic only. The first Windows
full attempt is separately inadmissible: its zero-sibling preflight passed,
then the package gate started another Windows pytest process. The full-suite
process tree was terminated and its evidence claim marked stale; no terminal
result from that attempt is accepted. Coverage and fresh-ext4 WSL qualification
never started. The next clean successor must rebuild package/install evidence
and restart Windows full, serialized subprocess-aware coverage, fresh-ext4 WSL
full, independent review, both live canaries, integrity-v2 seal, doctor,
exact-main remote CI, publication/readback, promotion, and ARISE installation.

### 2026-07-25 `596fae9` producer-version review rejection

The next clean successor,
`596fae97dbf425d13803cbae38ed84c49e14e90d`, tree
`95d4da2964e758ee8854fa0be1acb42dd9ae4d01`, is also rejected and must
not be promoted, pushed, tagged, published, or installed. A preliminary
exact-commit review reported P0/P1/P2 = 0/0/1 after comparing the actual
runner-v1, runner-v2, and runner-v3 producer implementations. The previous
repair correctly denied v3-only `WSL_DISTRO_NAME` and `WSL_INTEROP` on v1/v2,
but the shared fixed-environment set still allowed a resealed v1 receipt to
claim `PYTHONNOUSERSITE`. Runner v1 did not emit that name; runner v2
introduced it with the contained-pytest contract.

The successor freezes the complete receipt grammar by literal producer
version. Environment admission is:

- runner v1 admits the original ambient allowlist plus
  `PYTHONPATH`, `PYTHONDONTWRITEBYTECODE`,
  `PYTEST_DISABLE_PLUGIN_AUTOLOAD`, and `PYTHONHASHSEED`;
- runner v2 retains the same ambient allowlist and adds
  `PYTHONNOUSERSITE` to the fixed contained environment; and
- runner v3 retains runner v2's fixed set and additionally admits the two
  WSL routing names, subject to its current domain/distro/kernel cross-field
  validation.

Historical parsing does not gratuitously require names that the original
validator treated as optional, but it rejects names that the corresponding
producer could not emit. Positive fixtures preserve authentic v1/v2 read
compatibility; resealed negatives cover v1 with the v2-only name and v1/v2
with each v3-only WSL name.

Invocation schema selection, the structured-invocation digest protocol, and
argument-contract validation also dispatch through an immutable literal
version registry. They do not read the moving legacy/current role aliases.
A falsification test moves both role aliases and the current argument-contract
constant forward while proving that runner-v2/v3 receipts, their digest
payloads, and their `pytest-contained-argv-v2` contract remain unchanged.

The locked `596fae9` build produced package bytes and began the release O7
slice before the static review completed. The O7 process tree was terminated
after rejection, and all package bytes, partial logs, and review output are
diagnostic only. Windows full, subprocess-aware coverage, and fresh-ext4 WSL
full never started for this candidate. The next clean successor must rebuild
package/install evidence and restart every local, live-canary, integrity,
remote-CI, publication, promotion, and ARISE installation gate.
...
11. **Release rehearsal and exact remote gate** — full local
    fresh-ext4-WSL/Windows matrix, wheel/sdist installed smoke, independent
    rebuild, encrypted local manifest/bundle, exact pre-push receipt, then
    exact-main-push GitHub Linux/Windows test plus main-only docs. A tag test is
    supplementary; the publication workflow independently verifies the
    required main-push observations before its first Release mutation.
12. **Downstream installation boundary** — no installation or execution claim
    ...
    profile-required final environment evidence, immutable GitHub/PyPI
    readback, a Chief-created content-addressed promotion bundle, and
