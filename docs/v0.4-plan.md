...
historical Git executable identity. New endpoint capture writes
...
retains snapshot v2 for existing records, but mixed snapshot versions and
authority drift cannot elevate new evidence. The provenance scope is
...
- `tests/test_git_plumbing.py`: `63 passed, 39 subtests passed`;
- `tests/test_codex_transport_contracts.py` plus
  `tests/test_codex_transport_mutation.py`: `88 passed, 1 skipped`;
- all 40 `tests/test_codex_transport_cli.py` cases pass in seven isolated
  shards, including a hostile `git.exe`/non-absolute-argv guard for no-pin v2
  replay and the high-level writable crash/retry/replay case;
- `tests/test_codex_app_server_stdio.py` plus
  `tests/test_confidentiality.py`: `141 passed, 3 skipped`;
- targeted mypy passes for six canary/Git/Bridge/confidentiality implementation
  modules, all 221 tracked Python files compile from bytes in memory, and
  `git diff --check` passes.

These are dirty-byte contract/static checks only. They are not a clean commit,
package/install result, full Windows or fresh-ext4 WSL result, live App Server
canary, Git mutation verification, integrity review/seal, remote CI, or
