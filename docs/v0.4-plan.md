...
The writable live phase has an additional unresolved dependency: semantic-v2
Stage 1 does not yet have task-local `claim` and `release-claim` transitions.
A legacy sidecar claim cannot satisfy the mutation receipt's exact semantic
task-id binding. The structural writable runner test therefore proves command
ordering and Post binding only; it is not authorization or readiness to run a
writable live canary. That phase remains blocked until the semantic claim
transition is implemented, migrated, and independently tested.
