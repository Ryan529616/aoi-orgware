...
- CI is green on the release commit for Linux and Windows with Python 3.11,
  3.12, and 3.13, including the installed-artifact jobs.
...
<release-root>/build-env/bin/python -m twine check --strict \
  <release-root>/dist/*
<release-root>/build-env/bin/python scripts/verify_dist.py \
...
source tree. This verifies the built artifacts; it does not by itself establish
byte-reproducibility across independently resolved build toolchains.
...
the peeled tag commit. Then create the matching GitHub prerelease and publish
it. Publishing the release triggers `.github/workflows/publish.yml`; publishing
a draft also counts as the `published` event.
...
5. runs `scripts/verify_dist.py` against both artifacts;
6. uploads the verified pair as a same-run workflow artifact; and
7. passes only those bytes to the OIDC publication job.
