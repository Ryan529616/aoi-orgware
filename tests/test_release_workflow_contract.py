...
TEST_WORKFLOW=Path(__file__).resolve().parents[1]/".github"/"workflows"/"test.yml"
...
def _test_workflow()->str:return TEST_WORKFLOW.read_text(encoding="utf-8")
def _docs_workflow()->str:return DOCS_WORKFLOW.read_text(encoding="utf-8")
def _release_runbook()->str:return RELEASE_RUNBOOK.read_text(encoding="utf-8")
def _release_history()->str:return CHANGELOG.read_text(encoding="utf-8")+V04_PLAN.read_text(encoding="utf-8")
...
def _job_keys(body:str)->list[str]:
    lines,keys=body.splitlines(),[]
    pat=r'''"(?:\\.|[^"\\])*"|'(?:''|[^'])*'|.*?'''
    for n,line in enumerate(lines):
        if not re.match(r"^    \S",line) or line[4]=="#": continue
        text=line[4:]
        if text.startswith("?"):
            key=text[1:].strip()
            if re.fullmatch(r"[|>][0-9+-]*",key):
                ...
                for row in lines[n+1:]:
                    if row.startswith("      "): parts.append(row[6:])
                    elif row.strip(): break
                key=("\n" if key[0]=="|" else " ").join(parts).strip()
            else:
                match=re.fullmatch(fr"(?P<k>{pat})(?:\s+#.*)?",key)
                key=match["k"] if match else "\0"
        else:
            match=re.match(fr"(?P<k>{pat})(?=\s*:(?:\s|$))",text)
            key=match["k"] if match else "\0"
        if not key or key[0] in "&*!{[": key="\0"
        elif key[0]=="'": key=key[1:-1].replace("''","'")
        elif key[0]=='"':
...
def test_release_dag_has_one_linux_producer_then_windows_rebuild_observe() -> None:
    ...
    assert "needs: [verify-pypi-stage, producer-linux, verify-exact-ci]" in github_release
    release_readback = _job(text, "verify-github-release")
    assert "needs: [publish-github-release, verify-pypi-stage, producer-linux]" in release_readback
    publish = _job(text, "publish-pypi")
    assert "needs: [verify-pypi-stage, verify-github-release, producer-linux]" in publish
...
def test_producer_requires_an_annotated_exact_tag_and_all_checkouts_use_its_commit() -> None:
    ...
    assert "annotated tag object does not directly name the exact tag and commit" in _job(text, "assemble-observe")
...
def test_release_tag_runbook_fails_closed_before_exact_object_push() -> None:
    ...
    assert 'test "$(sha256sum "$tag_preflight" | awk \'{print $1}\')" = \\\n  "$tag_preflight_sha256" || exit 1' in text
    assert 'test "$(sha256sum "$tag_preflight_recheck" | awk \'{print $1}\')" = \\\n  "$tag_preflight_sha256" || exit 1' in text
    assert 'cmp --silent "$tag_preflight" "$tag_preflight_recheck" || exit 1' in text
    assert "--recorded-preflight-verification-index <tag-preflight-verification-index>" in text
    assert '--recorded-preflight-artifact-sha256 "$tag_preflight_sha256"' in text
...
def test_v132_release_tag_runbook_pins_transport_and_states_rewrite_race_boundary() -> None:
    ...
    assert "ambient rewrite observed before the network boundary is a failure before network access" in normalized_runbook
    assert "post-guard race cannot redirect the already pinned subprocess away from the exact endpoint" in normalized_runbook
    assert "must not be described as, an atomic lock over Git configuration" in normalized_runbook
    assert "14e2f9db8ce9506068bad456ce901bcec86d34a1403043de49e4ccfb81835e89" in history
    assert "P0=0/P1=1/P2=0" in history
    assert "109 passed, 2 subtests passed" in history
    assert "931ef80a342310e298f7f3fe2d3f3b48e94a943ae7d5e62b05ffe304149bcfbe" in history
    assert "220 passed, 8 skipped, 2 subtests passed" in history
    assert "31c1230cb449fb248114d910ab56791917805655b1ae865fe6c6d28dd5637ae2" in history
...
def test_v133_release_guard_and_network_use_same_normalized_config_authority() -> None:
    ...
    assert "ambient command-count, parameter, no-system, and system-file selectors are scrubbed" in normalized_runbook
    assert "temporary endpoint pins themselves are removed" in normalized_runbook
    assert "`GIT_CONFIG_NOSYSTEM=1` cannot hide a system rewrite" in normalized_runbook
    assert "cacfc7726af7680888f26bec4ef8deb76d30456cf3c416e5d37fae824ad18a2f" in history
    ...
    assert "d5cbcb2de77484ff99195fbc45fbea928939af394be631a4cec7fad58868c113" in normalized_history
    assert "224 passed, 8 skipped, 2 subtests passed" in normalized_history
    assert "6f0c2b28efbd2ab938e2e21ee895aca676a6f1403266aa634d0a682d9c091eab" in normalized_history
    assert "f148f8733dd6a2ec95d03619e38b8d2af3bab1c8cbdb174fe9e3824d61b05655" in normalized_history
    assert "P0=0/P1=0/P2=0" in normalized_history
    assert "permits full Windows/fresh-ext4 WSL qualification" in normalized_history
...
def test_v134_full_qualification_failure_is_not_reused_as_acceptance() -> None:
    ...
    assert "a8b1d723389e8965f290f27eee2f158fb6289c2238eda3baeb6f2e48d734a022" in history
    assert "ended `-1` after partial progress" in history
    assert "2ebcbe1e41704abcd2713ea34bc61ac0906266542c1f5612703cb95046d14b08" in history
    assert "Production behavior is unchanged" in history
    assert "96 passed, 1 skipped, 57 subtests passed" in history
    assert "d7b686ef301280ed0971e99970c9ab51774bf9a2403857d2a563a2e2a910db7e" in history
    assert "complete sequential Windows/WSL evidence was still pending at that checkpoint" in history
...
def test_v135_records_the_rejected_review_and_exact_command_correction() -> None:
    ...
    assert "716bbc1af8c08168a595c30ccaa2504b1db843c3683a5017df0c829de4e20fe7" in history
    ...
    assert "a311e27d47b7ddcf60df70e92b49415fd68c1476e0b956494225dfa793009794" in history
...
def test_v136_records_exact_native_full_qualification_without_promotion() -> None:
    ...
    assert "fbba27af8ccc15ad29731125165e05c475dc3600defb3b6b9f41575c0c385e0d" in history
    assert "1906 passed, 29 skipped in 1426.14s" in history
    assert "95308a8eb4855d2afe07aaab1bbbfb1ce7a17cb67f25f9bdd1dac622fdb65563" in history
    ...
    assert "do not establish independent review, integrity-v2 sealing, package/install acceptance, remote CI, tag creation, publication, promotion, or ARISE installation" in history
...
def test_github_release_is_gated_exact_and_verified_before_pypi() -> None:
    ...
    assert "artifact-ids: ${{ needs.verify-pypi-stage.outputs.github-release-artifact-id }}" in publisher
    ...
    stable_state_at = publisher.index('test "$release_state_after" = "$release_state_before"')
    tag_recheck_at = publisher.index("assert_tag_binding", stable_state_at)
    upload_missing_at = publisher.index('for name in "${missing_assets[@]}"; do')
    ...
    publish_draft_at = publisher.index('gh_api_json --method PATCH "repos/$GITHUB_REPOSITORY/releases/$release_id"')
    published_validation_at = publisher.index("GitHub Release was not published as a prerelease")
    final_download_at = publisher.index('final_readback="$RUNNER_TEMP/github-release-published-readback"')
    assert upload_missing_at < draft_readback_at < publish_draft_at < published_validation_at < final_download_at
    ...
    assert "needs: [verify-pypi-stage, verify-github-release, producer-linux]" in pypi
...
def test_release_mutation_requires_exact_successful_main_push_test_and_docs() -> None:
    ...
    assert '--workflow ".github/workflows/test.yml=$RUNNER_TEMP/test-runs.json"' in gate
    assert '--workflow ".github/workflows/docs.yml=$RUNNER_TEMP/docs-runs.json"' in gate
    assert "needs: [verify-pypi-stage, producer-linux, verify-exact-ci]" in publisher
    assert text.index("  verify-exact-ci:") < text.index("  publish-github-release:")


def test_full_ci_timeouts_cover_the_observed_long_matrix_runtime()->None:
    for j,n in(("unit",210),("coverage",180)):
        b=_job(_test_workflow(),j)
        assert "\0"not in(keys:=_job_keys(b))and keys.count("timeout-minutes")==1
        assert re.findall(r"^    timeout-minutes: (\d+)$",b,re.M)==[str(n)]
