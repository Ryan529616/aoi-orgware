...
MAX_PERMIT_CONSUMPTIONS = routing.MAX_ROUTING_ENTRIES
...
def validate_permit_consumption(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != _CONSUMPTION_FIELDS:
        raise PermitRuntimeError("permit consumption projection schema is invalid")
    ...
    if (
        item["action"] != "packet.arm"
        or not isinstance(target_ids, list)
        or len(target_ids) != 1
        or not isinstance(routing_slots, list)
        or len(routing_slots) != 1
        or routing_slots != sorted(set(routing_slots))
        or item["cohort_state"] is not None
    ):
        raise PermitRuntimeError("packet.arm permit consumption shape is invalid")
    checked = {
        "schema_version": PERMIT_RUNTIME_SCHEMA_VERSION,
        "permit_sha256": _sha(item["permit_sha256"], "consumed permit SHA-256"),
        "decision_sha256": _sha(item["decision_sha256"], "consumed decision SHA-256"),
        "replay_marker": _sha(item["replay_marker"], "consumed replay marker"),
        "action": "packet.arm",
        "target_ids": [h.validate_id(target_ids[0], "permit target id")],
        "routing_slots": [
            _sha(routing_slots[0], "consumed routing slot")
        ],
        "cohort_state": None,
    }
    _require_bounded_json(checked, permits.MAX_PERMIT_BYTES, "permit consumption")
    return checked
...
def validate_permit_namespace(value: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != _NAMESPACE_FIELDS:
        raise PermitRuntimeError("permit projection namespace schema is invalid")
    _exact_version(
    ...
    if (
        ...
        raise PermitRuntimeError("permit projection indexes are invalid or over bound")
    ...
    for identity, raw in consumptions.items():
        ...
        receipt = validate_permit_consumption(raw)
        checked_consumptions[identity] = receipt
    for marker, identity in replay_markers.items():
        ...
        if receipt is None or receipt["replay_marker"] != marker:
            raise PermitRuntimeError("permit replay-marker index cross-binding is invalid")
        checked_markers[marker] = identity
    if {
        receipt["replay_marker"] for receipt in checked_consumptions.values()
    } != set(checked_markers):
        raise PermitRuntimeError("permit consumption lacks a unique replay-marker index")
    checked = {
        "schema_version": PERMIT_RUNTIME_SCHEMA_VERSION,
        "consumptions": {
...
def _consumption_receipt(
    ...
    receipt = validate_permit_consumption(
        {
            "schema_version": PERMIT_RUNTIME_SCHEMA_VERSION,
            ...
            "routing_slots": [entry["outcome_slot_sha256"]],
            "cohort_state": None,
        }
    )
...
def _advance_permit_projection(
    ...
    if identity in namespace["consumptions"]:
        raise PermitRuntimeError("permit consumption identity is already committed")
    if marker in namespace["replay_markers"]:
        raise PermitRuntimeError("permit replay marker is already committed")
    if len(namespace["consumptions"]) >= MAX_PERMIT_CONSUMPTIONS:
        raise PermitRuntimeError("permit consumption projection reached its count bound")
    ...
    _require_bounded_json(
        domain, semantic.MAX_CANONICAL_JSON_BYTES, "permitted result projection"
    )
    return domain
...
def _publish_permit_issuance(
    ...
    rows, aggregate = _scan_permit_issuances(paths, checked["task_id"])
...
def _required_permit_issuance(
    ...
    rows, _aggregate = _scan_permit_issuances(paths, transaction["task_id"])
    matches = [
        marker
        for marker in rows
...
def issue_permitted_arm_transaction(
    ...
    *,
    ...
    markers, _aggregate = _scan_permit_issuances(paths, tx["task_id"])
    existing = next(
        (
            marker
...
def inspect_permit_runtime(
    ...
    issuance_rows, _issuance_bytes = _scan_permit_issuances(paths, task_id)
    ...
    for marker in issuance_rows:
        try:
            ...
            raise _fail(
                "permit issuance marker references a missing semantic object", exc
            ) from exc
    ...
    for row in binding_rows:
        ...
        if references & contract_digests and binding["binding_kind"] != "permit_consumption":
            raise PermitRuntimeError("permit contract object is referenced by another binding kind")
        ...
        try:
            ...
            raise _fail("permit binding references a missing semantic object", exc) from exc
        permit = group["permit"]
        decision = group["decision"]
        identity = permits.permit_consumption_identity(permit)
        ...
        route_slot = routing.routing_outcome_slot_sha256(group["arm"])
        expected_receipt = validate_permit_consumption(
            {
                "schema_version": PERMIT_RUNTIME_SCHEMA_VERSION,
                "permit_sha256": permit["permit_sha256"],
                "decision_sha256": decision["decision_sha256"],
        ...
        if classification == "committed":
            ...
            if (
                ...
                or committed_event["event_type"] != _EVENT_TYPE
